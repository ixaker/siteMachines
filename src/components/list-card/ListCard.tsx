...
const ListCard = () => {
  ...
  const editor = useSelector(selectEditor);
  const [loading, setLoading] = useState(true);
  const [visibleItems, setVisibleItems] = useState(INITIAL_ITEMS_COUNT);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        const [entry] = entries;
        if (entry.isIntersecting && visibleItems < list.length) {
          setVisibleItems((prev) => prev + ITEMS_PER_LOAD);
        }
      },
      {
        root: null, // Вся видимая область
        rootMargin: '0px',
        threshold: 1.0, // Элемент полностью в зоне видимости
      }
    );
  ...
  const displayedItems = list.slice(0, visibleItems);
