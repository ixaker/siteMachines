...
const Card: React.FC<CardProps> = ({ item }) => {
  ...
  return (
    <>
      {item ? (
        <div
          onClick={() => router.push(`/machine/?id=${item.id}`)}
          id={item.id}
        ...
        >
          ...
          <div className="flex flex-col pt-3 md:pt-0 gap-3 md:gap-[11px] md:justify-around md:h-full relative z-[0] sm:z-[1] group-hover:z-[3] bg-white sm:h-[150px] max-h-[150px] px-4 pb-2 rounded-b-lg group-hover:rounded-b-none transition-all duration-500 ease">
            <h2 className="text-sm md:text-lg font-bold text-gray-800 mt-1 h-[56px] overflow-hidden">{item.data?.name || ''}</h2>
            ...
            <div className="flex justify-between items-end">
              ...
              <span className="text-sm font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded-lg shadow-sm">Код: {item.data?.article || ''}</span>
            </div>
          </div>
